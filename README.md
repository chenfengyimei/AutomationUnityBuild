...
一套完整的游戏发布，从来不是"打个包"就结束的孤立动作，而是一条环环相扣的流水线。**AutomationUnityBuild 把这条流水线从手工经验固化成了可复用、可追溯、可扩展的系统能力**，覆盖从游戏开发到正式发布的每一个环节：

```
┌─────────────┐    ┌─────────────┐    ┌─────────────┐    ┌─────────────┐    ┌─────────────┐
│  游戏开发    │ →  │  自动化构建  │ →  │  上传测试    │ →  │  上传商店    │ →  │  发布游戏    │
│  (Unity)    │    │  (CLI/Web)  │    │ (TestFlight)│    │ (App Store) │    │ (灰度/正式) │
└─────────────┘    └─────────────┘    └─────────────┘    └─────────────┘    └─────────────┘
       ↑                                                                            │
       └──────────────────── 邮件通知 / 日志追溯 / 配置沉淀 ←─────────────────────────┘
```

| 阶段 | 传统做法 | AutomationUnityBuild 的做法 |
|------|---------|---------------------------|
| **游戏开发** | 开发完手动打开 Unity，点 Build，等半小时 | Git 仓库配置化，一键拉取最新代码，Unity BatchMode 无人值守构建 |
| **自动化构建** | 记命令、翻路径、找证书、手工看日志 | CLI 数字快捷指令 / Web 后台点按钮 / DesktopApp 桌面客户端，三种入口任选 |
| **上传测试** | 手动打开 Transporter，拖拽 `.ipa`，等上传，再进 App Store Connect 提交 | 构建完成自动调用 App Store Connect API，TestFlight 自动分发给测试组 |
| **上传商店** | 手工填写版本号、选择构建版本、提交审核 | 版本号自动 +1、Google Play 灰度发布比例可配、TikTok 开放平台 API 直传 |
| **发布游戏** | 群里发消息"包好了"，测试同学手动下载 | 邮件自动通知成功/失败、产物集中存储、历史构建可追溯、审计日志完整 |

### 闭环的价值

- **一套配置，处处可用**：项目/工程/版本/签名/证书五类模板沉淀后，新成员一键填充即可发起构建，不再依赖"只有某人知道怎么打包"
- **一次构建，多端触达**：同一份 Unity 工程，产出 iOS `.ipa` + Android `.apk/.aab` + TikTok 小游戏包，统一进入各自商店
- **一次失败，全程可查**：总日志、Unity 日志、Xcode 日志、Android 日志分层保存，配合邮件通知第一时间定位问题
- **一个入口，多种形态**：CLI 适合开发者、Web 适合团队协作、DesktopApp 适合离线操作、LinuxGateway 适合多机调度——同一套核心逻辑，四种使用姿势

这就是 AutomationUnityBuild 存在的意义：**让游戏团队的精力回归游戏本身，而不是消耗在重复的发包流程上。**
