...
internal static class InteractiveMenu
{
    public static async Task<int> RunAsync()
    {
        while (true)
        {
            ...
            Console.WriteLine("  1. 初始化新配置");
            Console.WriteLine("  2. 选择配置并开始打包/预览");
            Console.WriteLine("  3. 选择配置并检查环境");
            Console.WriteLine("  4. 查看已有配置");
            Console.WriteLine("  5. 手动输入完整命令");
            ...
            switch (choice?.Trim())
            {
                case "1":
                    ...
                    break;
                case "2":
                    await RunSelectedConfigAsync();
                    break;
                case "3":
                    await Cli.ExecuteAsync(["doctor"]);
                    break;
                case "4":
                ...
                case null:
                    return 0;
                default:
                    Console.WriteLine("请输入 1、2、3、4、5 或 0。");
                    break;
            }
        }
    }

    private static async Task RunSelectedConfigAsync()
    {
        bool defaultDryRun = !OperatingSystem.IsMacOS();
        bool dryRun = ConsolePrompts.AskBool("是否只预览命令 dry-run", defaultDryRun);
        ...
        if (dryRun)
        {
            ...
            {
                args.Add("--allow-non-mac");
            }
        }

        await Cli.ExecuteAsync(args.ToArray());
    }
}
