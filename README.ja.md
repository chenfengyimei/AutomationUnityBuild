...
[中文](README.md) | [English](README.en.md) | [日本語](README.ja.md) | [한국어](README.ko.md) | [Français](README.fr.md) | [Русский](README.ru.md) | [Español](README.es.md) | [Português](README.pt.md) | [完全利用ガイド](docs/usage.ja.md) | [アーキテクチャ](docs/architecture.ja.md)
...
完全なゲームリリースは、決して「ビルドして終わり」の単発作業ではなく、環環相扣くさ連鎖するパイプラインです。**AutomationUnityBuild はこのパイプラインを手作業の経験から、再利用可能・追跡可能・拡張可能なシステム能力へと昇華します**。ゲーム開発から正式リリースまでのすべての段階をカバーします：

```
┌─────────────┐    ┌─────────────┐    ┌─────────────┐    ┌─────────────┐    ┌─────────────┐
│ ゲーム開発   │ →  │ 自動ビルド   │ →  │ テスト配信   │ →  │ ストア申請   │ →  │ ゲーム公開   │
│  (Unity)    │    │  (CLI/Web)  │    │ (TestFlight)│    │ (App Store) │    │ (段階/本番) │
└─────────────┘    └─────────────┘    └─────────────┘    └─────────────┘    └─────────────┘
       ↑                                                                            │
       └──────────────────── メール通知 / ログ追跡 / 設定資産化 ←─────────────────────────┘
```

| 段階 | 従来の方法 | AutomationUnityBuild の方法 |
|------|---------|---------------------------|
| **ゲーム開発** | 開発後に手動で Unity を開き、Build をクリックして 30 分待つ | Git リポジトリを設定化し、ワンクリックで最新コードを取得、Unity BatchMode で無人ビルド |
| **自動ビルド** | コマンドを暗記し、パスを探し、証明書を探し、ログを目視確認 | CLI 数字ショートカット / Web ボタン操作 / DesktopApp クライアント、3 つの入口から選択 |
| **テスト配信** | Transporter を手動で開き、`.ipa` をドラッグしてアップロード待ち、App Store Connect で申請 | ビルド完了後に App Store Connect API を自動呼び出し、TestFlight がテストグループへ自動配布 |
| **ストア申請** | バージョン番号を手入力し、ビルドを選び、審査へ手動提出 | バージョン番号自動インクリメント、Google Play 段階的リリース比率を設定可能、TikTok オープンプラットフォーム API 直接アップロード |
| **ゲーム公開** | チャットで「パッケージできた」と連絡し、テスターが手動ダウンロード | 成功/失敗のメール自動通知、成果物の一元管理、ビルド履歴の追跡、完全な監査ログ |

### ループの価値

- **1 つの設定をどこでも**：プロジェクト/Unity/バージョン/署名/証明書の 5 種テンプレートを蓄積すれば、新メンバーもワンクリックで項目を埋めてビルド開始。「あの人しかビルド方法を知らない」から脱却
- **1 回のビルドで多プラットフォーム**：同じ Unity プロジェクトから iOS `.ipa` + Android `.apk/.aab` + TikTok ミニゲームを生成し、それぞれのストアへ
- **1 回の失敗もすべて追跡**：全体ログ、Unity ログ、Xcode ログ、Android ログを階層保存し、メール通知で即座に問題を特定
- **1 つの入口、多彩な形態**：CLI は開発者向け、Web はチーム協働向け、DesktopApp はオフライン作業向け、LinuxGateway はマルチマシン調整向け — 同じコアロジックで 4 つの使い方

これが AutomationUnityBuild の存在意義です：**ゲームチームの力をゲーム本体に戻し、繰り返しのリリース作業に消耗させない。**
...
また、TikTok ミニゲームの WebGL ビルドとオープンプラットフォーム API アップロード、メール通知（成功/失敗、SMTP 465 暗黙 SSL）、ストレージ管理（成果物クリーンアップ / ストレージ概要 / 一括削除）、5種類の設定テンプレート（プロジェクト / Unity / バージョン / 署名 / 証明書）、および MCP ツール経由での AI Agent のビルドプロセス参加機能もカバーしています。
...
| **設定テンプレート** | 5種テンプレート（プロジェクト / Unity / バージョン / 署名 / 証明書）、ワンクリックでフィールド入力、サーバー双方向同期をサポート | [テンプレート管理](docs/usage.ja.md#テンプレート管理) |
...
```mermaid
graph TB
    subgraph Entry["🚀 ユーザーエントリ"]
        CLI["💻 CLI ターミナル<br/>ショートカット · 対話ウィザード · ドライラン"]
        WebUI["🌐 BuildServer<br/>Web コンソール · タスクキュー"]
        Desktop["🖥️ DesktopApp<br/>Avalonia 11 デスクトップクライアント"]
        Gateway["🌍 LinuxGateway<br/>マルチノード公開エントリ"]
        Agent["🤖 MCP / Agent<br/>AI ツール制御呼び出し"]
    end

    subgraph Schedule["📋 スケジュール・管理"]
        Queue["シリアルタスクキュー"]
        Auth["ユーザー · 権限 · 監査"]
        Email["メール通知<br/>SMTP 465 暗黙 SSL"]
        Storage["ストレージ管理<br/>成果物クリーンアップ · 一括削除"]
        Templates["5種設定テンプレート<br/>プロジェクト / Unity / バージョン / 署名 / 証明書"]
        AutoUpdate["オンライン自己更新<br/>Gitee + GitHub デュアルソース"]
    end

    subgraph Engine["⚙️ ビルドエンジン"]
        Config["設定選択 · 編集 · スナップショット"]
        GitSync["Git リポジトリ同期<br/>ホワイトリスト · パス安全"]
        Unity["Unity BatchMode<br/>自動ビルド実行"]
        Logs["ログ · 設定スナップショット · 成果物ディレクトリ"]
    end

    subgraph Platforms["📱 プラットフォームビルド"]
        iOS["🍎 iOS<br/>Xcode archive / export"]
        Android["🤖 Android<br/>APK / AAB"]
        TikTok["🎵 TikTok<br/>WebGL ビルド"]
    end

    subgraph Stores["📦 ストア公開"]
        ASC["App Store Connect<br/>TestFlight 自動アップロード"]
        GP["Google Play<br/>Publishing API · 段階的ロールアウト"]
        TT["TikTok オープンプラットフォーム<br/>API アップロード"]
    end

    subgraph BuildNodes["🖥️ ビルドノード"]
        Mac["Mac ビルドマシン<br/>iOS · Android"]
        Win["Windows ノード<br/>Android"]
    end
